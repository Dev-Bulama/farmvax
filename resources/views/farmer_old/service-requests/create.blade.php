        ...
        <aside class="hidden md:flex md:flex-shrink-0">
            <div class="flex flex-col w-64 bg-brand-green">
                ...
                <nav class="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
                    ...
                    <a href="{{ route('farmer.service-requests') }}" class="flex items-center px-2 py-2 text-sm font-medium text-white bg-green-600 rounded-md">
                        <svg class="mr-3 h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <header class="bg-white shadow">
                <div class="px-4 sm:px-6 lg:px-8 py-4">
                    <div class="flex justify-between items-center">
                        <div>
                            <h1 class="text-2xl font-bold brand-teal">Request Veterinary Service</h1>
                            <p class="text-sm text-gray-600">Get professional help for your livestock</p>
                        </div>
                        <a href="{{ route('farmer.service-requests') }}" class="text-brand-green hover:underline text-sm font-medium">
                            ← Back to Requests
                        </a>
                    </div>
                </div>
            ...
            <main class="px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div class="max-w-3xl mx-auto mb-6">
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        <div class="flex">
                            ...
                            <div>
                                <p class="text-sm text-blue-800 font-medium mb-1">How Service Requests Work</p>
                                <ul class="text-sm text-blue-700 list-disc list-inside space-y-1">
                                    <li>Submit your request with details about the issue</li>
                                    <li>Our team will review and assign a professional</li>
                                    <li>You'll receive confirmation and estimated response time</li>
                ...
                <div class="max-w-3xl mx-auto bg-white rounded-lg shadow">
                    <div class="px-6 py-4 border-b border-gray-200">
                        <h2 class="text-lg font-bold brand-teal">Service Request Details</h2>
                    </div>

                    <form method="POST" action="{{ route('farmer.service-requests.store') }}" class="p-6">
                        @csrf
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                <label for="service_type" class="block text-sm font-medium text-gray-700 mb-2">
                                    Type of Service Needed <span class="text-red-500">*</span>
                                </label>
                                <select 
                                    name="service_type" 
                                    id="service_type" 
                                    required
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-green focus:border-brand-green"
                                >
                                    <option value="">Select service type...</option>
                                    <option value="vaccination" {{ old('service_type') == 'vaccination' ? 'selected' : '' }}>Vaccination</option>
                                    <option value="treatment" {{ old('service_type') == 'treatment' ? 'selected' : '' }}>Medical Treatment</option>
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                                    <label class="relative flex cursor-pointer rounded-lg border border-gray-300 bg-white p-4 hover:border-brand-green focus:outline-none">
                                        <input type="radio" name="urgency" value="low" {{ old('urgency') == 'low' ? 'checked' : '' }} class="sr-only" required>
                                        <span class="flex flex-1 flex-col">
                                            <span class="block text-sm font-medium text-gray-900">Low</span>
                                            <span class="mt-1 flex items-center text-xs text-gray-500">Can wait</span>
                                        </span>
                                        <svg class="h-5 w-5 text-brand-green hidden peer-checked:block" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                                        </svg>
                                    </label>

                                    <label class="relative flex cursor-pointer rounded-lg border border-gray-300 bg-white p-4 hover:border-brand-green focus:outline-none">
                                        <input type="radio" name="urgency" value="medium" {{ old('urgency') == 'medium' ? 'checked' : '' }} class="sr-only">
                                        <span class="flex flex-1 flex-col">
                                            <span class="block text-sm font-medium text-gray-900">Medium</span>
                                            <span class="mt-1 flex items-center text-xs text-gray-500">Within week</span>
                                        </span>
                                    </label>

                                    <label class="relative flex cursor-pointer rounded-lg border border-gray-300 bg-white p-4 hover:border-brand-green focus:outline-none">
                                        <input type="radio" name="urgency" value="high" {{ old('urgency') == 'high' ? 'checked' : '' }} class="sr-only">
                                        <span class="flex flex-1 flex-col">
                                            <span class="block text-sm font-medium text-gray-900">High</span>
                                            <span class="mt-1 flex items-center text-xs text-gray-500">1-2 days</span>
                                        </span>
                                    </label>

                                    <label class="relative flex cursor-pointer rounded-lg border-2 border-red-300 bg-red-50 p-4 hover:border-red-500 focus:outline-none">
                                        <input type="radio" name="urgency" value="critical" {{ old('urgency') == 'critical' ? 'checked' : '' }} class="sr-only">
                                        <span class="flex flex-1 flex-col">
                                            <span class="block text-sm font-medium text-red-900">Critical</span>
                                            <span class="mt-1 flex items-center text-xs text-red-700">Immediate</span>
                                        </span>
                                    </label>
                                </div>
                            </div>
...
- Which animal(s) are affected
- What symptoms you've observed
- How long this has been happening
- Any other relevant details..."
                                >{{ old('description') }}</textarea>
                                <p class="mt-2 text-sm text-gray-500">Be as specific as possible to help us serve you better</p>
                            </div>
                            ...
                            <div>
                                ...
                                <input 
                                    ...
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-green focus:border-brand-green"
                                >
                                <p class="mt-2 text-sm text-gray-500">If you have a preferred date, we'll try to accommodate it</p>
                            </div>

                        </div>

                        <!-- Emergency Notice -->
                        <div id="emergencyNotice" class="hidden mt-6 bg-red-50 border-l-4 border-red-400 p-4">
                            <div class="flex">
                                <svg class="h-5 w-5 text-red-400 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
                                </svg>
                                <div>
                                    <p class="text-sm font-medium text-red-800">Emergency Priority</p>
                                    <p class="text-sm text-red-700 mt-1">Your request will be marked as critical and our team will respond immediately. If life-threatening, please also call our emergency hotline: <strong>+234-XXX-XXX-XXXX</strong></p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-8 flex flex-col sm:flex-row gap-4">
                            ...
                            <a 
                                href="{{ route('farmer.service-requests') }}" 
    ...
    <script>
        // Show emergency notice when critical urgency is selected
