...
<div class="flex h-screen overflow-hidden">
    ...
    <aside id="sidebar" class="fixed md:static inset-y-0 left-0 transform -translate-x-full md:translate-x-0 transition-transform duration-300 ease-in-out z-50 md:z-auto w-64 bg-primary flex flex-col">
        ...
        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            ...
            <a href="{{ route('professional.farm-records') }}" class="flex items-center px-3 py-3 text-sm font-semibold text-white bg-white/20 rounded-lg border-l-4 border-white">
                <svg class="mr-3 h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                Farm Records
            </a>
            <a href="{{ route('professional.service-requests') }}" class="flex items-center px-3 py-3 text-sm font-medium text-white/90 hover:bg-white/10 rounded-lg transition">
                <svg class="mr-3 h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <header class="bg-white shadow-sm z-10">
            <div class="px-4 sm:px-6 lg:px-8 py-4">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('professional.farm-records.create') }}" class="hidden sm:flex items-center px-4 py-2 bg-primary text-white font-bold rounded-xl hover:bg-primary/90 transition">
        ...
        <main class="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-8 py-8 bg-gray-50">
            ...
            <div class="sm:hidden mb-6">
                <a href="{{ route('professional.farm-records.create') }}" class="flex items-center justify-center w-full px-4 py-3 bg-primary text-white font-bold rounded-xl hover:bg-primary/90 transition">
                    <svg class="h-5 w-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                    </svg>
                    Submit New Record
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100">
                    ...
                    <div class="hidden md:block overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-gray-700 uppercase">Farmer Name</th>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-gray-700 uppercase">Farm Name</th>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-gray-700 uppercase">Location</th>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-gray-700 uppercase">Livestock</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-gray-50 transition">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="h-10 w-10 bg-primary/10 rounded-full flex items-center justify-center">
                                                <span class="text-primary font-black text-sm">{{ strtoupper(substr($record->farmer_name, 0, 2)) }}</span>
                                            </div>
                                            <div class="ml-3">
                                                <p class="text-sm font-bold text-gray-900">{{ $record->farmer_name }}</p>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <a href="{{ route('professional.farm-records.show', $record->id) }}" class="text-primary font-semibold hover:text-primary/80">View →</a>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
                        ...
                        <div class="bg-gray-50 rounded-xl p-4">
                            <div class="flex items-start justify-between mb-3">
                                <div class="flex items-center flex-1">
                                    ...
                                    <div class="ml-3">
                                        <p class="text-sm font-bold text-gray-900">{{ $record->farmer_name }}</p>
                                        <p class="text-xs text-gray-500">{{ $record->farmer_city }}, {{ $record->farmer_state }}</p>
                                    </div>
                                </div>
                                @php
                                    $status = $record->status ?? 'draft';
                            ...
                            <div class="space-y-2 text-xs mb-3">
                                ...
                                <div class="flex justify-between">
                                    ...
                                    <span class="font-semibold text-gray-900">{{ $record->created_at->format('M d, Y') }}</span>
                                </div>
                            </div>
                            <a href="{{ route('professional.farm-records.show', $record->id) }}" class="block w-full text-center py-2 bg-white text-primary font-bold rounded-lg border-2 border-primary/20 hover:bg-primary hover:text-white transition">
                                View Details
                            </a>
                        </div>
                        @endforeach
                ...
                @else
                    <div class="text-center py-16">
                        ...
                        <h3 class="mt-2 text-sm font-bold text-gray-900">No farm records submitted</h3>
                        <p class="mt-1 text-sm text-gray-500">Get started by submitting your first farm data record.</p>
                        <div class="mt-6">
                            <a href="{{ route('professional.farm-records.create') }}" class="inline-flex items-center px-6 py-3 bg-primary text-white font-bold rounded-xl hover:bg-primary/90 transition">
                                <svg class="h-5 w-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                                </svg>
